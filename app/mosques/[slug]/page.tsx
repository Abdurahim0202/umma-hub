...
import { MapPin, Phone, Globe, Clock, CheckCircle, Calendar, ExternalLink } from 'lucide-react';
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-3 text-sm">
          ...
          {mosque.phone && (
            <a href={`tel:${mosque.phone}`} className="flex items-center gap-1.5 text-stone-600 hover:text-emerald-700">
              <Phone className="w-4 h-4 text-emerald-600" />
              {mosque.phone}
            </a>
          )}
          {mosque.website && (
            <a href={mosque.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-emerald-600 hover:text-emerald-800">
            ...
            {mosque.prayerTimes && (
              <div className="divide-y divide-stone-100">
                {PRAYER_ROWS.map(({ label, key }) => {
                  ...
                  const adhan = pt[key];
                  const iqamah = key !== 'sunrise' && mosque.iqamahTimes
                  ...
                  return (
                    <div key={label} className="px-4 py-2.5 flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <p className="text-xs text-stone-400">Adhan: <span className="text-stone-700 font-medium">{adhan}</span></p>
                        {iqamah && key !== 'sunrise' && (
      ...
      <div className="mt-8 bg-stone-100 rounded-2xl h-48 flex items-center justify-center text-stone-400 border border-stone-200">
        <div className="text-center">
          ...
          <a
            href={`https://maps.google.com/?q=${encodeURIComponent(mosque.address + ', ' + mosque.city + ', ' + mosque.state)}`}
            target="_blank"
